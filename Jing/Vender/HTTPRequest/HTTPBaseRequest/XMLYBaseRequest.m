...
- (NSURLSessionDataTask *)startWithMethod:(XMLYHTTPType)methodType
    ...
    if(methodType == XMLYHTTPTypeGET) {
        task = [self.sessionManager GET:[self.urlString stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]
                             parameters:params
                               progress:nil
        ...
        task = [self.sessionManager POST:[self.urlString stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]
                              parameters:params
                                progress:nil
                                 success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
                                     if(self.completionBlock) {
